...
class DataGenerator(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for idx, label in enumerate(labels):
            ...
            annotation[0, 14] = label[19]  # l4_x
            annotation[0, 15] = label[20]  # l4_y
            annotation[0, 16] = label[22]  # l4_x
            annotation[0, 17] = label[23]  # l4_y
            if (annotation[0, 4] < 0):
                annotation[0, 18] = -1
            else:
                annotation[0, 18] = 1
    ...
    def get_random_data(self, image, targes, input_shape, jitter=.3, hue=.1, sat=0.7, val=0.4):
        ...
        if len(box) > 0:
            ...
            box[:, [0, 2, 4, 6, 8, 10, 12, 14, 16]] = box[:, [0, 2, 4, 6, 8, 10, 12, 14, 16]] * nw / iw + dx
            box[:, [1, 3, 5, 7, 9, 11, 13, 15, 17]] = box[:, [1, 3, 5, 7, 9, 11, 13, 15, 17]] * nh / ih + dy
            if flip:
                # 这里翻转方式 改成我的标签关键点来翻转 p1-p2  u1-u2  o不变 u3-u4 
                box[:, [0, 2, 4, 6, 8, 10, 12, 14, 16]] = w - box[:, [2, 0, 6, 4, 10, 8, 12, 16, 14]]
                box[:, [5, 7, 9, 11, 13, 15, 17]] = box[:, [7, 5, 11, 9, 13, 17, 15]]
            ...
            box[:, [0, 2, 4, 6, 8, 10, 12, 14, 16]][box[:, [0, 2, 4, 6, 8, 10, 12, 14, 16]] > w] = w
            box[:, [1, 3, 5, 7, 9, 11, 13, 15, 17]][box[:, [1, 3, 5, 7, 9, 11, 13, 15, 17]] > h] = h
        ...
        box[:, 4:-1][box[:, -1] == -1] = 0
        box[:, [0, 2, 4, 6, 8, 10, 12, 14, 16]] /= w
        box[:, [1, 3, 5, 7, 9, 11, 13, 15, 17]] /= h
        box_data = box
